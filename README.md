...
1월 9일에 O(N^2)의 방법으로 풀고는 다음에 더 좋은 방법으로 풀어보자고 했다.
그게 오늘인가보다. 완전탐색 방법을 이용하고자 할 때 가장 신경써야하는 부분은
'시간 안에 풀 수 있는가?'이다. FENCE 문제의 경우 N이 20,000까지이므로
O(N^2)으로 푼다면 최악의 경우 4억이 나오는데 매우 러프하게 1억을 1초로 볼 수 있다고 했으므로,
4억이라면 문제의 요구사항인 1초를 만족시키지 못할 것이다. 더 빠른 풀이가 필요하다.

분할정복 방법을 사용할 수 있을까? 이 문제를 잘 읽어보면
문제를 더 작은 문제로(매번 거의 절반의 크기로 둘로 나누어) 분할한 뒤,
각 문제에 대한 답을 구하고 원래 문제에 대한 답으로 병합할 수 있는 문제이다.
게다가 더이상 분할하지 않고 바로 풀 수 있는 BASE CASE도 존재한다(하나의 판자만 존재할 때).
분할정복을 사용해보자. 가운데 판자를 기준으로 울타리를 절반으로 나누었을 때
직사각형이 최대가 되는 경우는 크게 두 가지이다. 왼쪽이든 오른쪽이든 한쪽에 몰려있거나,
왼쪽과 오른쪽 양쪽에 모두 걸치거나. 한쪽에 몰려있는 경우라면 재귀호출을 하여 답을 구해오면 되기 때문에
양쪽에 모두 걸친 경우만 잘 신경써주면 된다. 두 값 중 더 큰 값이 최댓값이다.

양쪽에 걸친 경우 가능한 모든 판자를 다따져서 만들 수 있는 직사각형을 다 만들려고 한다면
완전탐색 O(N^2) 보다 나을 게 없다. 최대일 수 있는 후보는 항상 왼쪽과 오른쪽 중에서 더 큰 판자를 골랐을 때
만들어지는 직사각형임을 이용해야 한다.
